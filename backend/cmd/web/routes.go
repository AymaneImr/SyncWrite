...
func RegisterRoutes(r *gin.Engine) {
	...
	{
		share.POST("/invite", utils.DocumentAccess(), handlers.InviteUser)             // works
		share.GET("/collaborators", utils.DocumentAccess(), handlers.GetCollaborators) // works
		share.PUT("/:userId/collaborator", utils.DocumentAccess(), handlers.UpdateCollaboratorPermission)
		share.DELETE("/:userId/collaborator", utils.DocumentAccess(), handlers.DeleteColabborator) // works
	}
